    ...
    <div className={`transition-colors duration-300`}>
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <h1 className={`text-2xl font-bold ${themed("text-slate-900", "text-white")}`}>Gambaran Umum</h1>
        </div>
        <div className={`flex items-center gap-2 p-1 rounded-full ${themed("bg-slate-200", "bg-slate-900")}`}>
          <FilterButton value="7d" label="7 Hari" />
          <FilterButton value="30d" label="30 Hari" />
          <FilterButton value="custom" label="Rentang Kustom" />
          <FiCalendar className={`mx-2 ${themed("text-slate-500", "text-slate-400")}`} />
        </div>
      </header>
