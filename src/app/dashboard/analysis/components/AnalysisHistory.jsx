...
function HistoryItem({ href, title, description, createdAt, recordCount, cohort, tag, isDark }) {
  ...
  const content = (
    <article className={`group min-w-0 rounded-2xl border p-4 transition-colors duration-300 ${itemShell}`}>
      <header className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className={`truncate text-sm font-semibold transition-colors ${isDark ? "text-slate-100" : "text-slate-800"}`}>{title}</h3>
          {description ? <p className={`mt-1 line-clamp-2 text-xs leading-relaxed ${isDark ? "text-slate-300/80" : "text-slate-600"}`}>{description}</p> : null}
        </div>
        {tag ? (
      ...
      <dl className={`mt-4 grid min-w-0 grid-cols-1 gap-3 text-xs sm:grid-cols-2 ${metaText}`}>
        <div className="flex items-center gap-2">
          <FiClock className={accent} />
          <div className="min-w-0">
            <dt className="font-medium uppercase tracking-[0.2em] opacity-75">Dibuat</dt>
            <dd className="mt-0.5 break-words">{createdAt}</dd>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <dd className="mt-0.5 break-words">{recordCount} kejadian</dd>
          </div>
        </div>
      </dl>
    </article>
  ...
  if (href) {
    return (
      <Link href={href} className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent">
        {content}
      </Link>
    );
  }
...
export default function AnalysisHistory({ title, subtitle, items, isDark }) {
  ...
  return (
    <section className={`w-full overflow-hidden rounded-3xl border p-6 shadow-sm transition-colors duration-300 ${shell}`}>
      <header className="flex items-start gap-3">
        <span className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl ${iconWrapper}`}>
          <FiBarChart2 size={22} />
        </span>
        <div className="min-w-0">
